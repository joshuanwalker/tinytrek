 ...
 *
 ***        **** STAR TREK ****        ****
 *** SIMULATION OF A MISSION OF THE STARSHIP Endeavour,
...
int k[4][4];                               /* Enemy Data */
int k3;                            /* Enemies in Quadrant */
int k7;                               /* Enemies at start */
int k9;                             /* Total Enemies left */
int n;                       /* Number of secors to travel */
int p;                            /* Proton Torpedoes left */
int p0 = 10;                    /* Proton Torpedo capacity */
int q1, q2;             /* Quadrant Position of Endeavour */
int r1, r2;              /* Temporary Location Corrdinates */
int s;                             /* Current shield value */
int s3;                               /* Stars in quadrant */
int s8;                         /* Quadrant locating index */
int s9 = 200;                             /* Enemy Power */
...
double s1, s2;     /* Current Sector Position of Endeavour */
...
initialize(void)
{
  ...
  printf("Destroy the %d Enemy warships that\n", k9);
  printf("have invaded the galaxy before they can\n");
  printf("attack Federation Headquarters on\n");
  printf("stardate %d. This gives you %d days.\n",  t0 + t9, t9);
  printf("There %s %d starbase%s in the galaxy\n", sX0, b9, sX);
...
new_quadrant(void)
{
  ...
  if (q1 >= 1 && q1 <= 8 && q2 >= 1 && q2 <= 8)
    {
      ...
      if (t0 != t)
      ...
      else
        {
          ...
          printf("located in the galactic quadrant\n");
          printf("%s.\n\n", sG2);
        }
    }
  ...
  /* Position Endeavour, then Enemies, Starbases, and stars */
  ...
  if (e - n < 0)
    {
      ...
      printf("  Insufficient energy available for\n");
      printf("   maneuvering at warp %4.1f!\n\n", w1);

      if (s >= n && d[7] >= 0.0)
        {
          printf("Deflector Control Room acknowledges:\n");
          printf("  %d units of energy presently\n", s );
          printf("  deployed to shields.\n");
        }
...
exceed_quadrant_limits(void)
{
  ...
  if (x5 == 1)
    {
      printf("Communication reports:\n");
      printf("  Message from Spacefleet Command:\n\n");
      printf("  Permission to attempt crossing of\n");
      printf("  galactic perimeter is hereby *denied*.\n");
      printf("  Shut down your engines.\n\n");
      printf("Chief Engineer reports:\n");
      /* @@@ printf("  Warp Engines shut down at sector %d, ", cint(s1)); */
      printf("  Warp Engines shut down at sector %d,\n", (int)s1);
...
maneuver_energy(void)
{
  ...
  printf("Shield Control supplies energy to\n");
  printf("complete maneuver.\n\n");
...
laser_control(void)
{
  ...
  if (d[4] < 0.0)
    {
      printf("Lasers Inoperative\n\n");
      return;
    }

  if (k3 <= 0)
    {
      printf("Science Officer reports:\n");
      printf("  'Sensors show no enemy ships in this\n");
      printf("   quadrant'\n\n");
      return;
    }
  ...
  printf("Lasers locked on target;\n");
      ...
      if (k[i][3] > 0)
        {
          ...
          if (h <= .15 * k[i][3])
            ...
            {
              k[i][3] = k[i][3] - h;
              printf("%d unit hit on Enemy at sector ", h);
              printf("%d, %d\n", k[i][1], k[i][2]);
              if (k[i][3] <= 0)
                {
...
proton_torpedoes(void)
{
  ...
  if (p <= 0)
    {
      printf("All proton torpedoes expended\n");
      return;
    }

  if (d[5] < 0.0)
    {
      printf("Proton Tubes not operational\n");
      return;
    }
  ...
  if (z3 == 1)
    {
      ...
      printf("Spacefleet Command reviewing your record\n");
      printf("to consider\n");
  ...
  if (d[6] < 0.0)
    {
      ...
      printf("repairs to your ship\n");
      /* @@@ printf("ship; Will you authorize the repair order (Y/N)? "); */
      printf("Estimated time to repair:\n");
      printf("%4.2f stardates.\n", d3);
      printf("Authorize repair order (Y/N)? ");
...
sheild_control(void)
{
  ...
  printf("  'Shields now at %d units\n", s);
  printf("   per your command.'\n\n");
}
...
status_report(void)
{
  ...
  printf("Enemy%s Left: %d\n", sX, k9);
  
  printf("Mission must be completed in %4.1f\n stardates\n",
    /* @@@ .1 * cint((t0 + t9 - t) * 10)); */
    .1 * (int)((t0 + t9 - t) * 10));
  ...
  if (b9 < 1)
  {
    ...
    printf("Your stupidity has left you on your own\n in the galaxy\n");
  ...
  {  
    ...
    printf("The Federation is maintaining %d \nstarbase%s in the galaxy\n",
      b9, sX);
  }
...
torpedo_data(void)
{
  ...
  {
    printf("Science Officer reports:\n");
    printf("  'Sensors show no enemy ships in \nthis quadrant.'\n\n");
    return;
  }
  ...
  printf("From Endeavour to Enemy\n battlecriuser%s:\n\n", sX);
...
nav_data(void)
{
  if (b3 <= 0)
  {
    ...
    printf("  'Sensors show no starbases in this\n quadrant.'\n\n");
    return;
  }
...
dirdist_calc(void)
{
  ...
  printf("You are at quadrant %d,%d\nsector %d,%d\n\n", q1, q2,
    /* @@@ cint(s1), cint(s2)); */
    (int)s1, (int)s2);
...
ship_destroyed(void)
{
  printf("The Endeavour has been destroyed. ");
...
{
  printf("There were %d Enemy Battlecruisers\nleft at the", k9);
...
won_game(void)
{
    ...
    printf("Congradulations, Captain!  The last\n");
    printf("Enemy Battle Cruiser menacing the\n");
    printf("Federation has been destoyed.\n\n");
  ...
  if (b9 > 0)
    {
      ...
      printf("The Federation needs a new starship\n");
      printf("commander for a similar mission.\n");
      printf("If there is a volunteer, let him step\n");
      printf("forward and  enter 'aye': ");
...
enemies_shoot(void)
{
  ...
  if (d0 != 0)
    {
      printf("Starbase shields protect the Endeavour\n\n");
      return;
    }
      ...
      if (k[i][3] > 0)
        {
          ...
          printf("%d unit hit on Endeavour from sector ", h);
          printf("%d, %d\n", k[i][1], k[i][2]);
...
get_device_name(void)
{
  static char * device_name[] = {
    ...
    "Laser Control","Proton Tubes","Damage Control","Sheild Control",
    "Library-Computer"};
